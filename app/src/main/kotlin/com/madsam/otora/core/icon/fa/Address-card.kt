...
public val Fa.`Address-card`: ImageVector
    get() {
        if (`_address-card` != null) {
            return `_address-card`!!
        }
        `_address-card` = Builder(name = "Address-card", defaultWidth = 640.0.dp, defaultHeight =
...
private var `_address-card`: ImageVector? = null
