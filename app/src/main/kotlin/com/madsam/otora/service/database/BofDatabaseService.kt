            ...
            try {
                ...
                comments.map { entry ->
                    ...
                    BofCommentUI(
                        ...
                        voteChartData = vote.sortedBy { it.score }.map { it.score }.toIntArray(),
                        short = entry.short,
                        shortTotal = entry.shortTotal,
                        shortAve = entry.shortAve,
                        shortComment = entry.shortComment,
                        shortChartData = short.sortedBy { it.score }.map { it.score }.toIntArray(),
                        long = entry.long,
                        longTotal = entry.longTotal,
                        longAve = entry.longAve,
                        longComment = entry.longComment,
                        longChartData = long.sortedBy { it.score }.map { it.score }.toIntArray(),
                        total = entry.total,
